...
describe('when there is initially some blogs saved', () => {
  ...
  describe('viewing a specific blog', () => {
    test('succeeds with a valid id', async () => {
      ...
      response.body.user = response.body.user.id.toString()
  ...
  describe('updating a blog', () => {
    ...
    test("succeeds updating only likes field if user tries to update another's blog", async () => {
      ...
      const response = await api.post('/api/login').send({ username: 'Dummy', password: 'dummy' })
    ...
    test('fails with status code 401 if user is not authenticated', async () => {
      ...
      await api.put(`/api/blogs/${kingsBlog.id}`).send({ title: 'Updated Blog', likes: 99 })
    })
  })
